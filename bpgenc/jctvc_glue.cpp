                         ...
                         const HEVCEncodeParams *params)
{
    ...
    strcpy(buf, "/tmp/");
#endif
    sprintf_s(infilename, sizeof(infilename), "%sout%d.yuv", buf, _getpid());
    sprintf_s(outfilename, sizeof(outfilename), "%sout%d.bin", buf, _getpid());
    ...
    sprintf_s(buf, sizeof(buf),"--InputFile=%s", infilename);
    add_opt(&argc, argv, buf);
    sprintf_s(buf, sizeof(buf),"--BitstreamFile=%s", outfilename);
    add_opt(&argc, argv, buf);

    sprintf_s(buf, sizeof(buf),"--SourceWidth=%d", img->w);
    add_opt(&argc, argv, buf);

    sprintf_s(buf, sizeof(buf),"--SourceWidth=%d", img->w);
    add_opt(&argc, argv, buf);
    sprintf_s(buf, sizeof(buf),"--SourceHeight=%d", img->h);
    add_opt(&argc, argv, buf);
    sprintf_s(buf, sizeof(buf),"--InputBitDepth=%d", img->bit_depth);
    add_opt(&argc, argv, buf);
    ...
    sprintf_s(buf, sizeof(buf),"--InputChromaFormat=%s", str);
    add_opt(&argc, argv, buf);

    sprintf_s(buf, sizeof(buf),"--QP=%d", params->qp);
    add_opt(&argc, argv, buf);
    
    sprintf_s(buf, sizeof(buf),"--SEIDecodedPictureHash=%d", 
