...
AVCodec ff_hevc_decoder = {
    "hevc", //const char *name;
    NULL_IF_CONFIG_SMALL("HEVC (High Efficiency Video Coding)"),//const char *long_name;
    AVMEDIA_TYPE_VIDEO,//enum AVMediaType type;
    AV_CODEC_ID_HEVC,//enum AVCodecID id;
	CODEC_CAP_DR1 | CODEC_CAP_DELAY | CODEC_CAP_SLICE_THREADS | CODEC_CAP_FRAME_THREADS,//int capabilities;
    NULL,//const AVRational *supported_framerates; ///< array of supported framerates, or NULL if any, array is terminated by {0,0}
    NULL,//const enum AVPixelFormat *pix_fmts;     ///< array of supported pixel formats, or NULL if unknown, array is terminated by -1
    NULL,//const int *supported_samplerates;       ///< array of supported audio samplerates, or NULL if unknown, array is terminated by 0
    NULL,//const enum AVSampleFormat *sample_fmts; ///< array of supported sample formats, or NULL if unknown, array is terminated by -1
    NULL,//const uint64_t *channel_layouts;         ///< array of support channel layouts, or NULL if unknown. array is terminated by 0
#if FF_API_LOWRES
    0,//uint8_t max_lowres;                     ///< maximum value for lowres supported by the decoder, no direct access, use av_codec_get_max_lowres()
#endif
    NULL,//const AVClass *priv_class;              ///< AVClass for the private context
    NULL,//const AVProfile *profiles;              ///< array of recognized profiles, or NULL if unknown, array is terminated by {FF_PROFILE_UNKNOWN}
    sizeof(HEVCContext),//int priv_data_size;
    ...
    hevc_decode_init,//int (*init)(AVCodecContext *);
    NULL,//int (*encode_sub)(AVCodecContext *, uint8_t *buf, int buf_size,const struct AVSubtitle *sub);
    NULL,//int (*encode2)(AVCodecContext *avctx, AVPacket *avpkt, const AVFrame *frame,int *got_packet_ptr);
    hevc_decode_frame,//int (*decode)(AVCodecContext *, void *outdata, int *outdata_size, AVPacket *avpkt);
    hevc_decode_free,//int (*close)(AVCodecContext *);
    hevc_decode_flush,//void (*flush)(AVCodecContext *);
};